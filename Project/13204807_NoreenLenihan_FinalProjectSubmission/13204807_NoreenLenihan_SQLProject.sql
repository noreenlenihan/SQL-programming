...
* 
* First, create workspace entitled 'PROJ_13204807' on the Application Express workspace
...
INSERT INTO proj_ship (ship_id, ship_name, ship_type, grt, ship_length, ship_speed, ship_capacity, route_id) VALUES
(NULL, 'Transformer at Sea', 'cruise ship', 150100, 1342, 25, 3800, 13);
INSERT INTO proj_ship (ship_id, ship_name, ship_type, grt, ship_length, ship_speed, ship_capacity, route_id) VALUES
...
INSERT INTO proj_ship (ship_id, ship_name, ship_type, grt, ship_length, ship_speed, ship_capacity, route_id) VALUES
(NULL, 'Perfection Ship', 'ocean liner', 261123, 1340, 36, 2700, 16);
INSERT INTO proj_ship (ship_id, ship_name, ship_type, grt, ship_length, ship_speed, ship_capacity, route_id) VALUES
(NULL, 'Zeus of the Sea', 'cruise ship', 251134, 1840, 33, 2200, 17);
INSERT INTO proj_ship (ship_id, ship_name, ship_type, grt, ship_length, ship_speed, ship_capacity, route_id) VALUES
(NULL, 'Hercules', 'ocean liner', 124880, 1250, 33, 1800, 18);
INSERT INTO proj_ship (ship_id, ship_name, ship_type, grt, ship_length, ship_speed, ship_capacity, route_id) VALUES
(NULL, 'Apollo', 'cruise ship', 166980, 1140, 25, 1200, 19);
INSERT INTO proj_ship (ship_id, ship_name, ship_type, grt, ship_length, ship_speed, ship_capacity, route_id) VALUES
(NULL, 'Dionysus', 'ocean liner', 116980, 2240, 29, 2100, 20);
INSERT INTO proj_ship (ship_id, ship_name, ship_type, grt, ship_length, ship_speed, ship_capacity, route_id) VALUES
(NULL, 'Venus', 'cruise ship', 136980, 1340, 36, 1970, 21);
...
COMMIT;
SELECT * FROM proj_routeport;

/* Part 2: Inner Joins */

/* More inner joins are produced than the actual requested amount. Examiner may choose preferred 4.*/
...
-- Show all the ships hosting packages and not hosting package in the system
...
/* Functions need to be implemented prior to procedures as the procedures invoke some of them */

-- Create view to facilitate queries 
-- Take all important information from proj_room, proj_reservation, proj_package, proj_room_status, and proj_room_details
-- and proj_membership tables for future reference 
...
num_reserv NUMBER(2);
-- User defined exceptions for when there are no rooms associated with reservation
ROOM_ASSIGN_ERROR EXCEPTION;
-- Associate custom function with ORA error code
PRAGMA EXCEPTION_INIT(ROOM_ASSIGN_ERROR, -20021);
-- Declare cursor and what it should represent (ie. FULLRESDETAILS_VW view where reservation_id is the passed reservation number)
CURSOR cur_reservation
IS
SELECT * FROM FULLRESDETAILS_VW WHERE reservation_id = reserv_nbr;
-- declare var to store each row returned by cursor of special rowtype datatype
reserv_row cur_reservation%ROWTYPE;
BEGIN
-- Ensure that there are rooms associated with reserv number
SELECT COUNT(reservation_id) INTO num_reserv FROM proj_room where reservation_id = reserv_nbr;
-- If not, then raise custom error
IF num_reserv = 0 THEN
	RAISE ROOM_ASSIGN_ERROR;
END IF;
-- Open cursor
OPEN cur_reservation;
-- Fetch current row of data into var reserv_row
FETCH cur_reservation INTO reserv_row;
WHILE cur_reservation%FOUND LOOP
-- Check payment offer type and calculate price accordingly
IF reserv_row.payment_offer = 'SALE' THEN
	-- if it was a lastmin deal, then customer pays 50% of base price
	new_pkg_price := (reserv_row.base_price * 0.50);
	-- don't forget to add on percent for membership type chosen
	reserv_total_cost := new_pkg_price + (new_pkg_price * (reserv_row.percent_add_on / 100));
-- if payment offer was full price, use full base price
ELSE
	-- calculate base price and add on for membership
	reserv_total_cost := reserv_row.base_price + (reserv_row.base_price * (reserv_row.percent_add_on / 100));
	-- print total per person per room
	DBMS_OUTPUT.PUT_LINE('Total per person per room is ' || reserv_total_cost);
	

END IF;
-- fetch next row
FETCH cur_reservation INTO reserv_row;
-- end while loop when there are no more data to fetch
END LOOP;
-- Close cursor
CLOSE cur_reservation;

-- Calculate total number of heads to charge for
SELECT sum(rm_capacity) INTO total_count FROM fullresdetails_vw where reservation_id = reserv_nbr;
-- print total number of people accounted for in reservation (dependent on room capacity, not actual people)
DBMS_OUTPUT.PUT_LINE('Total number of people in this reservation is ' || total_count);
-- multiply this number by total cost calculated thus far
reserv_total_cost := reserv_total_cost * total_count;
-- return this calue
RETURN reserv_total_cost;
-- exception section to account for when no rooms associated with reservation
EXCEPTION
WHEN ROOM_ASSIGN_ERROR THEN
	-- error message
	DBMS_OUTPUT.PUT_LINE('There are no rooms assigned for the passengers included in this reservation.');
	-- reraise exception to notify caller of error
	RAISE;
-- End function
...
RETURN NUMBER
IS
-- declare variables to store temp values
total_rev NUMBER(15,2);
actual_pkg NUMBER(3);
-- declare custom exception
PKG_DOES_NOT_EXIST EXCEPTION;
-- associate user defined exception with ora code
PRAGMA EXCEPTION_INIT(PKG_DOES_NOT_EXIST, -20022);
-- declare cursor as representing all the invoice values in the proj_reservation table for selected (passed) pkg id
CURSOR cur_reservation 
IS
SELECT invoice FROM proj_reservation WHERE package_id = package_num;
-- declare var to store rows returned by cursor
reserv_row cur_reservation%ROWTYPE;
BEGIN
-- first, check number of package ids for selected package number i.e. make sure pkg exists in proj_package table
SELECT COUNT(package_id) INTO actual_pkg FROM proj_package WHERE package_id = package_num;
-- if count is 0 then throw exception
IF actual_pkg = 0 THEN
	RAISE PKG_DOES_NOT_EXIST;
END IF;
-- initialized var to store total revenue
total_rev := 0;
-- open cursor
OPEN cur_reservation;
-- fetch row returned by cursor into var reserv_row
FETCH cur_reservation INTO reserv_row;
-- while there is a valid row returned, enter while loop
WHILE cur_reservation%FOUND LOOP
-- increment total_rev var to include new row's value for invoice
total_rev := total_rev + reserv_row.invoice;
-- fetch next row of data into reserv_row
FETCH cur_reservation INTO reserv_row;
-- end loop when no more data
END LOOP;
-- close cursor
CLOSE cur_reservation;
-- return value of total_rev to caller
RETURN total_rev;
-- exception section (after return statement)
EXCEPTION
WHEN PKG_DOES_NOT_EXIST THEN
	-- print error msg
	DBMS_OUTPUT.PUT_LINE('Invalid package number. Please enter a valid package ID.');
	-- reraise exception to notify caller
	RAISE;
-- end function
...
-- invoice customer with reservation 8
BEGIN
...
-- Create package specification to include 8 procedures
...
-- declare vars to store temp data
num_rooms_reset NUMBER(5);
date_pkg_expired DATE;
count_pkgs NUMBER;
BEGIN
-- make savepoint before any updates or modifications of table in case of error
SAVEPOINT before_reset;
-- get number of packages with this ID (should always be 1, never 0)
SELECT COUNT(package_id) INTO count_pkgs FROM proj_package where package_id = package_num;
-- if no pkg exists, then throw exception
IF count_pkgs = 0 THEN
	RAISE INVALID_PACKAGE_NBR;
END IF;
-- get arrival date of package in question
SELECT arrival_date INTO date_pkg_expired FROM proj_package WHERE package_id = package_num;
-- obtain number of rooms reserved in that package
...
-- if package arrival date is less than current date, it has expired
IF sysdate > date_pkg_expired THEN 
	-- reset reservation_id fields to null for relevant ship
...
-- rollback to right after begin statement when pkg does not exist (explicit rollback)
...
PROCEDURE RoomAssignment(reserv_num NUMBER, membership_chosen NVARCHAR2, qty_adults NUMBER, qty_child NUMBER) 
IS 
-- declare vars and any custom exceptions
total_qty NUMBER(3);
-- exceptions for >8 persons booking
TOO_MANY_PERSONS EXCEPTION;
-- exception for no one involved in booking
NO_PERSONS_BOOKED EXCEPTION;
PRAGMA EXCEPTION_INIT(TOO_MANY_PERSONS, -20006);
PRAGMA EXCEPTION_INIT(NO_PERSONS_BOOKED, -20008);
-- declare cursor and what it represented (a join between proj_room and proj_room_status)
CURSOR cur_room
IS
...
-- declare var to store rows returned by cursors with special rowtype datatype
room_row cur_room%ROWTYPE;

BEGIN
-- add qty_child and qty_adults to obtain total quantity
SELECT (qty_child + qty_adults) INTO total_qty FROM dual;
-- check for violation of reservation rules
IF total_qty < 1 THEN
...
-- open cursor
OPEN cur_room;
-- fetch current row into room_row car
FETCH cur_room INTO room_row;
-- check while loop continuation loop and if qty of heads is still above 0
WHILE cur_room%FOUND AND total_qty > 0 LOOP
	-- make savepoint in case of error
	SAVEPOINT before_room_assignment;
	-- keep track of decrementing head count
	DBMS_OUTPUT.PUT_LINE('qty of ppl: ' || total_qty);
	-- if statement for when total persons is less or equal to 8 and there is at least one child (family room in here)
	IF total_qty <= 8 and qty_child >= 1 then
		-- for family room, there must be at least 5 people included in reservation
		IF room_row.reservation_id IS NULL AND room_row.room_type = 'family room' AND total_qty = 5 THEN
		-- if entered, assign rooms using assignRoom procedure
		assignRoom(room_Row.room_id, reserv_num);
		-- decrement head count by 5
		total_qty := total_qty - 5;
		-- repeat logic for when there are still people left to assign rooms for..
		...
		END IF;
	-- condition for when there are no kids involved and so the family room is not an option
	ELSIF total_qty <= 8 AND total_qty > 0 AND  qty_child = 0 THEN
		...
		total_qty := total_qty - 1;
		END IF;
	-- if total qty greater than 8 then raise error message
...
PROCEDURE ReservationCancellation(reserv_num NUMBER)
IS
-- declare variables for use in procedures
...
-- declare var to store each row returned by cursor
...
-- check if reservation actually exists
...
PROCEDURE assignRoom(room_num NUMBER, reserv_num NUMBER)
IS
-- declare vars to store local values
...
BEGIN
-- check if room number and reservation number exist in tables
...
ELSIF real_res = 0 THEN
	RAISE INVALID_RESRV;
ELSE
	-- ready to assign room to this reservation id
...
PROCEDURE LastMinPackageDeal(package_num NUMBER)
IS
-- declare vars to store temp values
...
-- check if package ID actually exists
IF valid_pkg_count = 0 THEN
	RAISE INVALID_PKG_FOR_DEAL;
END IF;
-- get number of people that actually have reservations for this package
...
-- get number of potential reservations for this package
...
ELSE
	-- else notify that it is ineligible for reduction
	DBMS_OUTPUT.PUT_LINE('Package number ' || package_num || ' is ineligible for a last-minute-package-deal.');
END IF;
-- exception section
EXCEPTION 
WHEN INVALID_PKG_FOR_DEAL THEN
	DBMS_OUTPUT.PUT_LINE('This package does not exist.');
	-- reraise exception 
	RAISE;
...
cust_lname NVARCHAR2, cust_addr NVARCHAR2, cust_phone NUMBER, cust_email NVARCHAR2, membership_chosen NVARCHAR2, qty_adults NUMBER, qty_child NUMBER)
IS
-- declare vars to store local data
reduction NVARCHAR2(2);
payment_type NVARCHAR2(10);
reserv_num NUMBER(10);
valid_pkg_nbr NUMBER(3);
total_heads NUMBER(3);
-- declare exceptions
INVALID_PKG_CHOICE EXCEPTION;
NO_PERSONS_RESERV EXCEPTION;
LEGAL_GUARDIAN EXCEPTION;
INVALID_MEM_CHOICE EXCEPTION;
-- associate exceptions with ora codes
PRAGMA EXCEPTION_INIT(INVALID_PKG_CHOICE, -20014);
PRAGMA EXCEPTION_INIT(NO_PERSONS_RESERV, -20015);
PRAGMA EXCEPTION_INIT(LEGAL_GUARDIAN, -20016);
PRAGMA EXCEPTION_INIT(INVALID_MEM_CHOICE, -20017);
BEGIN
-- check if valid membership type was specified
IF membership_chosen NOT IN ('platinum','gold','silver','bronze') THEN
	RAISE INVALID_MEM_CHOICE;
END IF;
SELECT COUNT(package_id) INTO valid_pkg_nbr FROM proj_package WHERE package_id = package_num;
-- check if pkg exists
IF valid_pkg_nbr = 0 THEN
	RAISE INVALID_PKG_CHOICE;
END IF;
-- obtain total qty of persons in reservation
SELECT (qty_adults + qty_child) INTO total_heads FROM DUAL;
IF total_heads = 0 THEN
	RAISE NO_PERSONS_RESERV;
END IF;
-- check whether child are not travelling alone
IF qty_child > 0 AND qty_adults = 0 THEN
	RAISE LEGAL_GUARDIAN;
END IF;
-- get status of whether pkg is on offer or not
SELECT reduced_price INTO reduction FROM proj_package WHERE package_id = package_num;
-- make insert for customer table
INSERT INTO proj_customer VALUES (NULL, cust_fname, cust_lname, cust_addr, cust_phone, cust_email);
-- make insert for reservation table
...
-- identify payment type by type of pkg (last minute or not)
IF reduction = 'Y' THEN
	payment_type := 'SALE';
ELSE
	payment_type := 'FULL PRICE';
END IF;
-- update reservation table's payment offer column with this value
UPDATE proj_reservation SET payment_offer = payment_type WHERE reservation_id = reserv_num;
-- invoke roomAssignment procedure to automatically assign rooms for passengers
RoomAssignment(proj_reserv_seq.currval, membership_chosen, qty_adults, qty_child);
-- exception section
EXCEPTION
WHEN INVALID_PKG_CHOICE THEN
	DBMS_OUTPUT.PUT_LINE('Invalid package choice. Please enter a valid package.');
	RAISE;
WHEN NO_PERSONS_RESERV THEN
	DBMS_OUTPUT.PUT_LINE('There must be at least ONE person involved in a booking.');
	RAISE;
WHEN LEGAL_GUARDIAN THEN
	DBMS_OUTPUT.PUT_LINE('All children must be accompanied by at least one adult for cruise holidays with RCI.');
	RAISE;
WHEN INVALID_MEM_CHOICE THEN
	DBMS_OUTPUT.PUT_LINE('Invalid membership type specified. Please enter a membership type of platinum OR silver OR gold OR bronze.');
	RAISE;
END FullReservation;
...
IS
-- declare vars to store local data
selected_ship NUMBER(3);
INVALID_SHIP EXCEPTION; 
PRAGMA EXCEPTION_INIT(INVALID_SHIP, -20018);
ship_depart_date DATE;
-- use cursor to traverse relevant records
CURSOR cur_select_pkg IS
SELECT * FROM pkgs_ship_assign_vw WHERE ship_num = ship_nbr;
pkg_row cur_select_pkg%ROWTYPE;
BEGIN
SELECT COUNT(ship_id) INTO selected_ship FROM proj_ship WHERE ship_id = ship_nbr;
-- see if ship passed in arg actually exists
IF selected_ship = 0 THEN
	RAISE INVALID_SHIP;
END IF;
-- get departure date of pkg associated with ship
...
-- if package does not have a ship already then assign the ship to this pkg
...
-- if the current data's arrival date is after the date the ship must depart then say so..
...
ELSE
-- else set this ship to be assigne with the pkg desired
UPDATE proj_package SET ship_id = ship_nbr WHERE package_id = package_num;
END IF;
FETCH cur_select_pkg INTO pkg_row;
END LOOP;
...
-- exception section
EXCEPTION
WHEN INVALID_SHIP THEN
	DBMS_OUTPUT.PUT_LINE('Invalid ship number. Please select a valid ship.');
	RAISE;
-- end procedure
END ShipAssignment;

-- Simple procedure to invoice customer holding a reservation for a pacakge
PROCEDURE InvoiceCustomer(reserv_num NUMBER)
IS
-- declare vars
billing NUMBER(10,2);
...
INVALID_RSV EXCEPTION;
PRAGMA EXCEPTION_INIT(INVALID_RSV, -20020);
BEGIN
SELECT COUNT(reservation_id) INTO actual_rsv FROM proj_reservation WHERE reservation_id = reserv_num;
-- does reservation exist in table?
IF actual_rsv = 0 THEN
	RAISE INVALID_RSV;
END IF;
-- get total cost of reservation by invoking function (above)
billing := CalculateTotalReservationCost(reserv_num);
-- now, set the invoice field equal to the result of this function for this reservation
UPDATE proj_reservation SET invoice = billing WHERE reservation_id = reserv_num;
-- print out message
DBMS_OUTPUT.PUT_LINE('Customer invoiced to the amount of ' || billing || '.');
-- exception section
EXCEPTION
WHEN INVALID_RSV THEN
	DBMS_OUTPUT.PUT_LINE('This is not a valid reservation number.');
	RAISE;
END InvoiceCustomer;
...
-- should fire before change to package_id on reservation table
...
BEGIN
-- get start date of package in question
SELECT depart_date INTO package_start_date FROM proj_package WHERE package_id = :NEW.package_id;
-- if package start date is before current date..they missed the boat
...
-- should fire AFTER table has been modified - we cannot affect change with an after trigger, simply a warning is sufficient
AFTER INSERT OR UPDATE OF ARRIVAL_DATE ON PROJ_PACKAGE
FOR EACH ROW
DECLARE
-- declare variables
pkg_duration NUMBER;
PKG_EXCESSIVE_DUR EXCEPTION;
PRAGMA EXCEPTION_INIT(PKG_EXCESSIVE_DUR, -20023);
BEGIN
-- Check length of package
pkg_duration := :NEW.arrival_date - :NEW.depart_date;
-- if length is greater than 40 days, issue warnign
IF pkg_duration > 40 THEN
	RAISE PKG_EXCESSIVE_DUR;
ELSE
	-- if not, print out a normal message
	DBMS_OUTPUT.PUT_LINE('Package number ' || :NEW.package_id || ' runs for ' || pkg_duration || ' which is normal duration. ');
END IF;
-- exception section
EXCEPTION
WHEN PKG_EXCESSIVE_DUR THEN
...
-- should fire when change to arrival_date data
...
BEGIN
-- if arrival is after depart date then thats normal, reiterate dates
...
-- else print out error message and raise exception
ELSE
...
-- A ship can never change its colors (or route) concordant with business rules
CREATE OR REPLACE TRIGGER SHIP_ROUTE_ASSIGNMENT
BEFORE INSERT OR UPDATE OF ROUTE_ID ON PROJ_SHIP
FOR EACH ROW
-- only used for when ship already had a route and is now assigned another
WHEN (OLD.ROUTE_ID IS NOT NULL AND NEW.ROUTE_ID IS NOT NULL)
DECLARE
-- declare custom excpetions
SHIP_ROUTE_IMMUTABLE EXCEPTION;
PRAGMA EXCEPTION_INIT(SHIP_ROUTE_IMMUTABLE, -20005);
BEGIN
-- if ship's old route and new route diverge then this is not allowed by business rules and throw exception
...
-- Good for updating or changing ship assignments to packages
-- This procedure is necessary because a package's route/destination is defined by the
-- ship that it is associated with, so if for example, 'Dubai Nights Winter 2014' is allocated ship 2
-- and ship 2 travels the Asian route, then this is a subtle but grossly erroneous modification
CREATE OR REPLACE TRIGGER PKG_CONSISTENT_SHIP_RT
BEFORE INSERT OR UPDATE OF SHIP_ID ON PROJ_PACKAGE
FOR EACH ROW
-- while fire only for packages that are changing from one ship to another
...
BEGIN
-- find out old and new routes of ships and compare
...
-- if dissimilar, then raise alarm
...
ELSE
	-- otherwise, issue message of change
...
BEGIN
RCI_OPERATIONS_PKG.ShipAssignment(15,2);
END;
/
