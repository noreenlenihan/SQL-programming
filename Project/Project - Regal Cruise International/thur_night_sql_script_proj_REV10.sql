...
PKG_ALREADY_HOSTED EXCEPTION;
PRAGMA EXCEPTION_INIT(INVALID_SHIP, -20018);
PRAGMA EXCEPTION_INIT(PKG_ALREADY_HOSTED, -20019);
...
IF selected_ship = 0 THEN
	RAISE INVALID_SHIP;
END IF;
SELECT ship_id INTO pkg_hosted FROM proj_package WHERE package_id = package_num;
IF pkg_hosted IS NOT NULL THEN
	RAISE PKG_ALREADY_HOSTED;
END IF;
...
WHEN INVALID_SHIP THEN
	...
	RAISE;
WHEN PKG_ALREADY_HOSTED THEN
	DBMS_OUTPUT.PUT_LINE('This package is already hosted by a ship. It cannot be reassigned to another ship.');
	RAISE;
END ShipAssignment;
...
-- Good for updating or changing ship assignments to packages (when you don't use the procedure)
